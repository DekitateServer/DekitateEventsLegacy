...
class EventTicketHistoryDao(dataSource: DataSource) : Dao(dataSource) {
    ...
    fun insert(uuid: UUID, amount: Int): Boolean {
        try {
            ...
            Log.error("Failed to insert a event ticket history", e)
        }
    ...
    fun getPositiveAmountBetween(uuid: UUID, start: Timestamp, end: Timestamp): Int {
        try {
            ...
            Log.error("Failed to select event ticket amount", e)
        }
